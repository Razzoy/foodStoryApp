...
export function CreateRecipe() {
  ...
  const handleRemoveImage = (indexToRemove:number) => {
    const newScreenshots = mealImage.filter((mealImage,index)=>index!==indexToRemove)
 ...
 const handleAddScreenShot = (image:string) => {
  ...
  const handleRemoveScreenshot = (indexToRemove:number) => {
    const newScreenshots = screenShots.filter((screenShot,index)=>index!==indexToRemove)
...
for (var p of data) {
  ...
  return (
    <form onSubmit={saveData}>
      <PhotoCard test="first" imageUrls={mealImage} handleAdd={handleAddImages} handleRemoveImage={handleRemoveImage}/>
      <input
      name="title"
      ...
      <div className="flex flex-center justify-center  gap-28  ">
          ...
          <div className=" flex flex-col items-center">
            <img className="flex -center w-12 p-2" src="src/assets/icons/Muscle.svg" />
            <Select name="select">
          ...
          <div className=" flex flex-col items-center">
            ...
            <input className="outline-none w-[50px]" placeholder="Time" name="time"/>
          </div>
      ...
      <div className="flex flex-col items-center gap-4 ">
        ...
        <button className="bg-customGreen cursor-pointer border-2 border-customGreen hover:bg-white  hover:border-2 hover: hover:text-black py-1 px-6 rounded-lg">Save</button>
      </div>
      </form>
  );
}
